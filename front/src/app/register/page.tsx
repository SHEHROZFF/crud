...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://crud-78ii.vercel.app/api/users/register', { // Update with your server URL
        method: 'POST',
        headers: {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 px-4">
      <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-md">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            <label htmlFor="promotionYear" className="block text-sm font-medium text-gray-600">Promotion Year</label>
            <input
              type="text"
              id="promotionYear"
              name="promotionYear"
          ...
          <div>
            <label htmlFor="association" className="block text-sm font-medium text-gray-600">Association</label>
            <input
              type="text"
              id="association"
              name="association"
          ...
          <div>
            <label htmlFor="assoc" className="block text-sm font-medium text-gray-600">Assoc</label>
            <input
              type="text"
              id="assoc"
              name="assoc"
              value={formData.assoc}
              onChange={handleChange}
              required
              placeholder="Assoc Info"
              className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <button
